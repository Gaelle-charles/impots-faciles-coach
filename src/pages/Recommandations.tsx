...
function ExpandableText({ text, clampClass }: { text: string; clampClass: string }) {
  ...
  // Heuristique : on propose "Voir plus" si le texte est suffisamment long
  // pour potentiellement déborder de la zone clampée.
  const canExpand = text.length > 110;
  return (
    <div>
      <p className={`text-sm text-muted-foreground leading-relaxed ${expanded || !canExpand ? '' : clampClass}`}>
        {text}
      </p>
      {canExpand && (
        <button
...
function RecoCard({ reco }: { reco: Reco }) {
  return (
    <Card className="border border-border bg-background rounded-3xl shadow-none hover:shadow-lg transition-all overflow-hidden">
      <CardContent className="p-5 flex flex-col lg:flex-row lg:items-start gap-5">
        {/* Logo */}
        {reco.logo_url ? (
          <img
            src={reco.logo_url}
            alt={reco.nom}
            className="h-14 w-14 shrink-0 rounded-2xl object-contain bg-white border border-border lg:mt-1"
          />
        ) : (
          <div className="h-14 w-14 shrink-0 rounded-2xl flex items-center justify-center font-heading font-bold text-sm bg-rose-light text-rose-dynamic lg:mt-1">
        ...
        <div className="flex-1 min-w-0 space-y-1">
          ...
          <ExpandableText text={reco.description} clampClass="line-clamp-2" />
        </div>
        ...
        <div className="lg:w-72 shrink-0 rounded-2xl p-3 text-sm bg-rose-light text-foreground">
          ...
          <ExpandableText text={reco.benefice_user} clampClass="line-clamp-3" />
        </div>

        {/* CTA */}
        <a href={reco.url} target="_blank" rel="noopener noreferrer" className="shrink-0 lg:mt-1">
